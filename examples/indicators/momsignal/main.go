// Example: momsignal signal — emits +100 (buy), -100 (sell), or 0 (hold).
package main
...
func main() {
	...
	out, err := gotal.Get("MOMSIGNAL")(ts)
	if err != nil {
		log.Fatal(err)
	}

	sig := out.Fields().Get("momsignal").Values()
	var buys, sells int
	for i, v := range sig {
		switch v {
		case 100:
	...
	fmt.Printf("# momsignal: %d buys, %d sells across %d ticks\n", buys, sells, len(sig))
}
