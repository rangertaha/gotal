// Example: cmosignal signal — emits +100 (buy), -100 (sell), or 0 (hold).
package main
...
func main() {
	...
	out, err := gotal.Get("CMOSIGNAL")(ts)
	if err != nil {
		log.Fatal(err)
	}

	sig := out.Fields().Get("cmosignal").Values()
	var buys, sells int
	for i, v := range sig {
		switch v {
		case 100:
	...
	fmt.Printf("# cmosignal: %d buys, %d sells across %d ticks\n", buys, sells, len(sig))
}
