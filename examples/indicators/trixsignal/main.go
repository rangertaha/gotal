// Example: trixsignal signal — emits +100 (buy), -100 (sell), or 0 (hold).
package main
...
func main() {
	...
	out, err := gotal.Get("TRIXSIGNAL")(ts)
	if err != nil {
		log.Fatal(err)
	}

	sig := out.Fields().Get("trixsignal").Values()
	var buys, sells int
	for i, v := range sig {
		switch v {
		case 100:
	...
	fmt.Printf("# trixsignal: %d buys, %d sells across %d ticks\n", buys, sells, len(sig))
}
