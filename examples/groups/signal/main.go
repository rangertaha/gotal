// Example: gotal.Signal runs every signal generator and reports buy/sell counts.
package main
...
func main() {
	...
	fmt.Printf("# Signal ran on %d ticks — buy/sell counts per signal:\n", out.Len())
	for _, name := range names {
		vals := out.Fields().Get(name).Values()
		var buys, sells int
		for _, v := range vals {
			switch v {
			case 100:
